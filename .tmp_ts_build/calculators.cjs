...
exports.categories = exports.calculatorLogic = void 0;
exports.getAllCalculators = getAllCalculators;
exports.searchCalculators = searchCalculators;
const lucide_react_1 = require("lucide-react");
/* ---------- CALCULATION LOGIC ---------- */
exports.calculatorLogic = {
    mortgage: {
        inputs: [
        ...
        calculate: ({ price, buyerType }) => {
            ...
            if (buyerType === 1) {
                if (price <= 500000)
                    ...
                    duty = (price - 500000) * 0.10;
            }
            else if (buyerType === 2) {
                if (price <= 1000000)
                    duty = price * 0.01;
                else
                    duty = 10000 + (price - 1000000) * 0.02;
            }
            else {
                duty = price * 0.025;
            }
    ...
    "gpa-calculator": {
        ...
        calculate: (values) => {
            ...
            function percentToGradePoint(p) {
                if (p >= 85)
                    return 4.0;
                if (p >= 80)
                    return 3.7;
            ...
            for (let i = 1; i <= subjects; i++) {
                ...
                let gp = 0;
                if (mode === 1)
                    gp = g; // already grade point
                else if (mode === 2)
                    gp = percentToGradePoint(g);
                else if (mode === 3)
                    gp = letterToGradePoint(String(g)); // letter grade
                totalQuality += gp * c;
                totalCredits += c;
            }
    ...
    "ucd-gpa": {
        ...
        calculate: (vals) => {
            ...
            function pctToUcdPoint(p) { if (p >= 70)
                return 4.0; if (p >= 60)
                return 3.0; if (p >= 50)
                return 2.0; return 0; }
            ...
            for (let i = 1; i <= n; i++) {
                const p = Number(vals[`m${i}`] || 0);
                const c = Number(vals[`c${i}`] || 5);
                const gp = pctToUcdPoint(p);
                totalQ += gp * c;
                totalC += c;
            }
    ...
    "usc-calculator": {
        ...
        calculate: ({ grossIncome, uscBand1Limit, uscRate1, uscBand2Limit, uscRate2, uscBand3Limit, uscRate3, uscRate4 }) => {
            ...
            if (remaining > 0)
                totalUSC += remaining * (uscRate4 / 100);
    ...
    "non-programmable": {
        ...
        calculate: ({ inputValue, divisor, simpleOp }) => {
            ...
            if (simpleOp === 1)
                ...
                result = Math.sqrt(Math.max(0, inputValue));
            else if (simpleOp === 3)
                result = divisor !== 0 ? inputValue / divisor : 0;
            else if (simpleOp === 4)
